...
- 2026-05-26 M10 scoped type audit truth：当前 2026-06-08 options i18n PR/main 合并树 fresh 实测为 overall `0/1113/1819/38/4`；src `0/613/628/7/0`；tests `0/500/1191/31/4`；`lint:type-any:ratchet` 必须同时守住 checked-in overall、src 与 tests 上限，tests 下降不得抵消 src 增长
...
- 2026-05-26 M10 budget ratchet truth：`lint:type-any:ratchet` 以 checked-in 上限 overall `0/1125/1838/53/4`、src `0/613/628/7/0`、tests `0/513/1216/46/4` 接入 `quality`；当前 fresh type counts 低于该上限时，不得把 checked-in 上限误写为 fresh 实测；`audit:performance:report` 动态发现当前全部 `95` 个 `src` >250 LOC 文件并要求每个文件存在 line budget（当前注册 `113` 个 line budgets）
