...
- 2026-06-01 Plan 03 R3 non-production source truth：`audit:non-production-source:check` 当前 decision counts 为 `migrate-import-owner: 145`、`migrate-script-owner: 2`、`migrate-test-owner: 2`、`retain-production: 532`、`retain-production-facade: 15`；任何 `src` 删除仍必须满足六项 exact-path owner proof
