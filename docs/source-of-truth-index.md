...
- 2026-05-24 gap-remediation baseline sync 真值：lint warning baseline 曾同步为 `254`；2026-05-26 M10 后 baseline 收紧为 `159`；2026-05-28 Plan 09 后 baseline 收紧为 `141`；2026-06-01 Plan 06 section retirement 后当前 checked-in baseline 为 `137`，fresh warning count 为 `136`，不得通过禁用规则或修改 lint 配置制造下降
...
- 2026-05-29 Plan 11 G4 non-production source truth：`audit:non-production-source:report` 当前 decision counts 为 `migrate-import-owner: 200`、`retain-production: 536`、`retain-production-facade: 15`；任何 `src` 删除仍必须满足六项 exact-path owner proof
