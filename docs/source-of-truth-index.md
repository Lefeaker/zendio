...
- 2026-06-05 GA production telemetry P13 performance-budget truth：当前 GA 集成树 `audit:performance:report` 动态发现 `99` 个 `src` >250 LOC 热点路径，注册 `100` 个 line budgets；完整预算真值以 [`performance-baseline.md`](./performance-baseline.md) 与 `tools/report-performance-hotspots.mjs` 为准
