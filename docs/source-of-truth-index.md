...
- 2026-06-05 GA / i18n PR merge build/performance truth：当前合并树 `audit:performance:report` 为 trackedSourceFiles=`693`，动态发现 `99` 个 `src` >250 LOC 热点路径，注册 `102` 个 line budgets；完整预算真值以 [`performance-baseline.md`](./performance-baseline.md) 与 `tools/report-performance-hotspots.mjs` 为准
...
- 2026-05-24 gap-remediation baseline sync 真值：lint warning baseline 曾同步为 `254`；2026-05-26 M10 后 baseline 收紧为 `159`；2026-05-28 Plan 09 后 baseline 收紧为 `141`；2026-06-01 Plan 06 section retirement 后 baseline 为 `137`；2026-06-01 Plan 03 native YAML retirement 后 checked-in baseline 仍为 `132`，而 2026-06-05 GA/i18n PR merge 后 fresh warning count 已下降到 `125`；在 owner 有意同步 baseline 前，不得通过禁用规则或修改 lint 配置制造下降
...
- 2026-05-26 M10 scoped type audit truth：当前 2026-06-05 GA/i18n PR merge 实测为 overall `0/1084/1776/41/4`；src `0/588/606/5/0`；tests `0/496/1170/36/4`；`lint:type-any:ratchet` 必须同时守住 checked-in overall、src 与 tests 上限，tests 下降不得抵消 src 增长
...
- 2026-05-26 M10 budget ratchet truth：`lint:type-any:ratchet` 以 checked-in 上限 overall `0/1084/1776/41/4`、src `0/588/606/5/0`、tests `0/496/1170/36/4` 接入 `quality`；`audit:performance:report` 动态发现当前全部 `99` 个 `src` >250 LOC 文件并要求每个文件存在 line budget（当前注册 `102` 个 line budgets）
