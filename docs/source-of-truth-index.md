...
- 2026-05-24 gap-remediation baseline sync 真值：lint warning baseline 曾同步为 `254`；2026-05-26 M10 后 baseline 收紧为 `159`；2026-05-28 Plan 09 后 baseline 收紧为 `141`；2026-06-01 Plan 06 section retirement 后 baseline 为 `137`；2026-06-01 Plan 03 native YAML retirement 后 checked-in baseline 为 `132`；2026-06-06 session-draft current-main reintegration baseline 为 `148`；2026-06-07 video legacy recovery 集成树已同步 checked-in baseline 为 `147`，当前 gate 输出为 `Warning 总量保持在基线 147 条`；不得通过禁用规则或修改 lint 配置制造下降
...
- 2026-05-26 M10 scoped type audit truth：当前 2026-06-07 video legacy recovery 集成树实测为 overall `0/1125/1838/53/4`；src `0/612/622/7/0`；tests `0/513/1216/46/4`；`lint:type-any:ratchet` 必须同时守住 checked-in overall、src 与 tests 上限，tests 下降不得抵消 src 增长
...
- 2026-05-26 M10 budget ratchet truth：`lint:type-any:ratchet` 以 checked-in 上限 overall `0/1125/1838/53/4`、src `0/612/622/7/0`、tests `0/513/1216/46/4` 接入 `quality`；`audit:performance:report` 动态发现当前全部 `105` 个 `src` >250 LOC 文件并要求每个文件存在 line budget（当前注册 `108` 个 line budgets）
