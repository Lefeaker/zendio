...
- 2026-06-01 non-production source report closure truth：先运行 `npm run audit:production-build-graph:report` 生成 `build/reports/production-build-graph.json`；随后 `audit:non-production-source:report` 当前退出 `0`，2026-06-05 GA/i18n PR merge 复核后 decision counts 为 `migrate-import-owner: 130`、`retain-production: 569`、`retain-production-facade: 17`。Plan 09 遗留的 4 个 completion-blocking rows 已收口：`readingTemplateControls.ts` 与 `ModalController.ts` 已六证据删除，`src/ui/foundation/keyboard/index.ts` 与 `src/ui/hosts/options/index.ts` 已按当前架构 owner 精确 retained-contract 分类。任何未来 `src` 删除仍必须满足六项 exact-path owner proof。
