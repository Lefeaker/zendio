...
- GA production release public config 真值：owner 本机使用 ignored `.env.production.local` 注入 `AIIINOB_GA_MEASUREMENT_ID`、`AIIINOB_GA_TRANSPORT_MODE`、`AIIINOB_GA_PROXY_ENDPOINT`；复用命令为 `analytics:validate:prod`、`build:prod:ga`、`package:prod:ga`、`package:firefox:prod:ga`、`release:prod:ga`。GA `api_secret` 仍只允许存在于 Cloudflare Worker secret `GA4_API_SECRET`。
...
- 2026-06-05 GA production telemetry P13 type-ratchet truth：当前 GA release env close-out `lint:type-any` 扫描 `1041` files，实测 overall `0/1064/1727/41/4`、src `0/576/592/5/0`、tests `0/488/1135/36/4`；`lint:type-any:ratchet` 仍守住 checked-in 上限 overall `0/1064/1736/41/4`、src `0/576/596/5/0`、tests `0/488/1140/36/4`；本次只同步 current truth，没有放宽 `any` 或 `ts-expect-error` 上限，`non-null` 上限继续保持 overall `41` / src `5` / tests `36`
- 2026-06-05 GA production telemetry P13 final build/performance truth：当前 GA release env close-out `quality` 与 `verify:preflight` 已在 Node `v20.20.2` 下通过；dev build `content/runtime.js` 为 `54.9 KB`（raw `56,170` bytes；raw stop gate `56,320`），`onboarding/index.js` 为 `15.8 KB`（raw `16,200` bytes），chunk count `109`；`audit:performance:report` 当前 trackedSourceFiles=`675`，动态发现 `98` 个 `src` >250 LOC 热点路径，注册 `100` 个 line budgets；完整预算真值以 [`performance-baseline.md`](./performance-baseline.md) 与 `tools/report-performance-hotspots.mjs` 为准
...
- 2026-05-24 gap-remediation baseline sync 真值：lint warning baseline 曾同步为 `254`；2026-05-26 M10 后 baseline 收紧为 `159`；2026-05-28 Plan 09 后 baseline 收紧为 `141`；2026-06-01 Plan 06 section retirement 后 baseline 为 `137`；2026-06-01 Plan 03 native YAML retirement 后 checked-in baseline 仍为 `132`，而 2026-06-05 GA release env close-out 后 fresh warning count 已下降到 `126`；在 owner 有意同步 baseline 前，不得通过禁用规则或修改 lint 配置制造下降
...
- 2026-05-26 M10 scoped type audit truth：当前 2026-06-05 GA release env close-out 实测为 overall `0/1064/1727/41/4`；src `0/576/592/5/0`；tests `0/488/1135/36/4`；`lint:type-any:ratchet` 必须同时守住 checked-in overall、src 与 tests 上限，tests 下降不得抵消 src 增长
- 2026-06-01 non-production source report closure truth：先运行 `npm run audit:production-build-graph:report` 生成 `build/reports/production-build-graph.json`；随后 `audit:non-production-source:report` 当前退出 `0`，2026-06-05 GA release env close-out 复核后 decision counts 为 `migrate-import-owner: 124`、`retain-production: 557`、`retain-production-facade: 17`。Plan 09 遗留的 4 个 completion-blocking rows 已收口：`readingTemplateControls.ts` 与 `ModalController.ts` 已六证据删除，`src/ui/foundation/keyboard/index.ts` 与 `src/ui/hosts/options/index.ts` 已按当前架构 owner 精确 retained-contract 分类。任何未来 `src` 删除仍必须满足六项 exact-path owner proof。
...
- 2026-05-26 M10 budget ratchet truth：`lint:type-any:ratchet` 以 checked-in 上限 overall `0/1064/1736/41/4`、src `0/576/596/5/0`、tests `0/488/1140/36/4` 接入 `quality`，当前实测低于 assertion 上限；`audit:performance:report` 动态发现当前全部 `98` 个 `src` >250 LOC 文件并要求每个文件存在 line budget（当前注册 `100` 个 line budgets）
