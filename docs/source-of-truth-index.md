...
- 2026-05-26 M10 scoped type audit truth：`lint:type-any` 扫描 `1091` files；2026-05-28 Plan 09 后 current overall `0/970/1661/108/5`；src `0/539/623/5/0`；tests `0/431/1038/103/5`；`lint:type-any:ratchet` 必须同时守住 overall、src 与 tests，tests 下降不得抵消 src 增长
- 2026-05-26 M10 non-production source truth：`audit:non-production-source:report` 当前 decision counts 为 `migrate-import-owner: 200`、`retain-production: 527`、`retain-production-facade: 15`；任何 `src` 删除仍必须满足六项 exact-path owner proof
...
- 2026-05-26 M10 budget ratchet truth：`lint:type-any:ratchet` 以 overall `0/970/1661/108/5`、src `0/539/623/5/0`、tests `0/431/1038/103/5` 接入 `quality`；`audit:performance:report` 动态发现当前全部 `99` 个 `src` >250 LOC 文件并要求每个文件存在 line budget
