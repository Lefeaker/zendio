...
- 2026-05-29 Plan 10 D3 dependency-audit current truth：Node `v20.20.2` / npm `10.8.2` 下，`npm audit --omit=dev` 为 `0` vulnerabilities，production runtime release gate 仍为 green；schema/release batch 后 `npm audit --audit-level=low` 当前仍失败，为 `2` vulnerabilities（`1` moderate / `1` high），后续由 Plan 10 Batch C 处理
