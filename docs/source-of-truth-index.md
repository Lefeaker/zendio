...
- 2026-06-05 session-draft P08 final integration type-ratchet truth：当前 session-draft integrated tree `lint:type-any` 扫描 `1060` files，实测 overall `0/1124/1812/53/4`、src `0/602/615/7/0`、tests `0/522/1197/46/4`；`lint:type-any:ratchet` 已同步 checked-in 上限 overall `0/1124/1812/53/4`、src `0/602/615/7/0`、tests `0/522/1197/46/4`；本次只同步 current truth，没有放宽 `any` 或 `ts-expect-error` 上限
...
- 2026-05-24 gap-remediation baseline sync 真值：lint warning baseline 曾同步为 `254`；2026-05-26 M10 后 baseline 收紧为 `159`；2026-05-28 Plan 09 后 baseline 收紧为 `141`；2026-06-01 Plan 06 section retirement 后 baseline 为 `137`；2026-06-01 Plan 03 native YAML retirement 后 baseline 为 `132`；2026-06-05 session-draft P08 final integration 已将 checked-in baseline 同步到 `146`，当前 fresh warning count 同为 `146`；在 owner 有意同步 baseline 前，不得通过禁用规则或修改 lint 配置制造下降
...
- 2026-05-26 M10 scoped type audit truth：当前 2026-06-05 session-draft integrated tree 实测为 overall `0/1124/1812/53/4`；src `0/602/615/7/0`；tests `0/522/1197/46/4`；`lint:type-any:ratchet` 必须同时守住 checked-in overall、src 与 tests 上限，tests 下降不得抵消 src 增长
...
- 2026-05-26 M10 budget ratchet truth：`lint:type-any:ratchet` 以 checked-in 上限 overall `0/1124/1812/53/4`、src `0/602/615/7/0`、tests `0/522/1197/46/4` 接入 `quality`；`audit:performance:report` 动态发现当前全部 `98` 个 `src` >250 LOC 文件并要求每个文件存在 line budget（当前注册 `100` 个 line budgets）
