...
- **匿名使用统计与错误报告（可选）**：仅在您于隐私设置中明确开启 `analytics` 或 `errorReporting` 后，扩展才会通过维护者控制的代理端点向 GA4 发送受限事件。扩展侧仅持有公开配置（`measurementId`、`transportMode`、`proxyEndpoint`），不持有也不发送 `api_secret`。`Usage analytics` 控制产品/使用事件；`Error reporting` 只控制 `extension_error` 类错误事件。
...
- 在扩展隐私设置中分别开启或关闭 `Usage analytics`、`Error reporting` 和 debug mode；
