...
- **账户与配置数据**：包括您在扩展设置中保存的 Obsidian REST API 地址、API Token、导出路径、模板规则、本地 Vault 目录授权标识等，仅保存在浏览器本地。
...
- **访问原因**：在 Chromium 浏览器中，您可以在设置页为每个 Vault 显式选择一个本地目录，使扩展在 Obsidian 未打开时也能写入 Markdown 和附件。
- **授权方式**：本功能使用浏览器提供的 File System Access API。扩展不会通过字符串路径扫描磁盘，只能访问您通过系统目录选择器授权的目录。Chrome 重启后如果目录权限变为待确认状态，扩展会在您再次触发剪藏、阅读模式导出或视频模式导出时，在当前页面请求重新授权；授权后当次操作会继续写入本地目录。
- **回退策略**：当已授权目录可写时，扩展优先写入本地目录；如果目录权限失效、目录不可用、浏览器不支持该 API，或您选择不再询问，则回退到 Obsidian Local REST API。
- **撤销授权**：您可以在扩展设置中清除本地目录配置，也可以通过浏览器站点/扩展数据管理清除已保存的目录授权。
...
- **撤销授权**：在 Obsidian 客户端中重置或撤销 REST API Token，可立即阻止本扩展继续访问。
...
- 本地目录写入仅限用户授权的目录，且仅在用户触发剪藏、导出或同步动作时执行。
