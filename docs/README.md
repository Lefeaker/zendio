...
- Firefox release tooling 不依赖 `web-ext`，但在本地锁定 `addons-linter 10.10.0`：`npm run lint:firefox:addons` 与 `package:firefox*` 共用 bounded self-hosted lint owner，package 顺序固定为仓库静态检查 → addons-linter → XPI → archive audit。上游不安全的 `image-size` 闭包由真实命名的 tracked metadata adapter 替代，warnings/notices 保持可见且不按成功隐藏；AMO upload validation 仍是商店提交阶段的独立校验。发布使用 first-party AMO API v5 adapter，exact-XPI smoke 使用 pinned geckodriver `0.37.1` 与 WebDriver BiDi install/bootstrap/uninstall/reinstall identity 契约
- i18n production copy 当前不变量：production user-visible copy 只能来自 i18n catalog 或 `UserVisibleMessageDescriptor` key；background/content 边界传 descriptor/code/params；`quality` 已包含 CJK/descriptor hard gate `audit:i18n-hardcoded-user-copy:check`，English uncatalogued-copy hard gate `audit:i18n-uncatalogued-user-copy:check` 已接入 `quality` 与 `verify:preflight`，覆盖 raw English `defaultMessage` fallback 与 `subtitle` / `hint` / `body` 等 production-visible fields。当前 production graph、scan、finding 与 allowlist rule 数值不在入口页冻结；先刷新 `audit:production-build-graph:report`，再运行两组既有 report/check owner，并以同一 fresh tree 的实际输出为准
