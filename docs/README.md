...
- i18n production copy 当前不变量：production user-visible copy 只能来自 i18n catalog 或 `UserVisibleMessageDescriptor` key；background/content 边界传 descriptor/code/params；`quality` 已包含 CJK/descriptor hard gate `audit:i18n-hardcoded-user-copy:check`，当前 audit truth 为 `scanned=578 findings=19 unexpected=0 staleAllowlist=0`；English uncatalogued-copy hard gate `audit:i18n-uncatalogued-user-copy:check` 已接入 `quality` 与 `verify:preflight`，当前 audit truth 为 `scanned=574 findings=0 unexpected=0 staleAllowlist=0`
