...
9. 单独规划 dev/release toolchain dependency upgrade
   ...
   - 当前真值：`npm audit --audit-level=low` 仍失败，2026-05-29 D3 glob batch 后当前报告为 `5` vulnerabilities（`2` moderate / `3` high）
   - 历史口径：2026-05-20 release readiness handoff 中的 `26` vulnerabilities（`10` moderate / `16` high）仅保留为当时证据，不代表当前 audit truth
   - 风险范围：dev/release toolchain，当前 vulnerable package names 为 `ajv`、`fast-uri`、`lodash`、`node-forge`、`ws`
