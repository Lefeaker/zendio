...
- `tools/baselines/lint-warnings.json` 基线仍记录历史 warning 债务；2026-06-01 Plan 03 native YAML retirement 后 checked-in baseline 仍为 `132` 条，而 2026-06-05 fresh warning count 已下降到 `127`。`lint:warnings-report` 仍会重写该 baseline，只能在有意同步 warning truth 时运行。
