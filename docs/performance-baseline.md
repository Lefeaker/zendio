...
- `build/dist/content/runtime.js`: `48.2 KB`（raw `49,325` bytes）
- `build/dist/options/index.js`: `1.1 KB`
- `build/dist/onboarding/index.js`: `9.4 KB`（raw `9,606` bytes）
- 总 chunk 数：`95`
- `chunks/runtimeEntry-*.js`: `122.2 KB`
- `chunks/videoLazyRuntime-*.js`: `79.4 KB`
- `chunks/videoSessionControllers-*.js`: `57.9 KB`
- `chunks/videoScreenshotPreparationQueue-*.js`: `14.8 KB`

当前 dev build 真值：

- `build/dist/content/index.js`: `561 B`
- `build/dist/content/runtime.js`: `55.8 KB`（raw `57,179` bytes；warning target `57,209` raw bytes；hard stop `57,386` raw bytes）
- `build/dist/options/index.js`: `1.5 KB`
- `build/dist/onboarding/index.js`: `16.1 KB`（raw `16,485` bytes；warning target `16,459` raw bytes；hard stop `16,715` raw bytes）
- 总 chunk 数：`111`（warning target `118`；hard stop `120`）
- `chunks/runtimeEntry-*.js`: `242.0 KB`
- `chunks/videoSessionControllers-*.js`: `109.7 KB`
- `chunks/videoLazyRuntime-*.js`: `55.5 KB`
- `chunks/messages-*.js`: `202.4 KB`
- `chunks/videoScreenshotPreparationQueue-*.js`: `29.4 KB`

当前 shared chunk Top 3（`chunk-*`，按 `tools/report-build-splitting.mjs` 口径，以 dev build 为更高值）：

- 最大 shared chunk：`135.3 KB`
- 第二大 shared chunk：`96.3 KB`
- 第三大 shared chunk：`95.5 KB`
...
- `chunks/openaiFamily-*.js`: dev `289 B` / production `203 B`
- `chunks/chineseFamily-*.js`: dev `318 B` / production `226 B`
- `chunks/assistantFamily-*.js`: dev `264 B` / production `180 B`
- `chunks/gemini-*.js`: dev `215 B` / production `146 B`
...
- `src/content/video/videoScreenshotCacheRepository.ts`: `420` 行
...
- `src/background/listeners/runtimeMessages.ts`: `324` 行
- `src/background/services/videoScreenshotCacheIndexedDbStore.ts`: `335` 行
...
- 全部当前 `src` >250 LOC 文件均有 guarded line budget；2026-06-17 P08 fresh `audit:performance:report` 输出 sourceFiles=`790`、hotspotsOver250=`95`、registeredLineBudgets=`120`，预算以 `tools/report-performance-hotspots.mjs` 为准。
...
- 当前高信号热点实测：`stitch/content.ts = 867`、`messages.generated.ts = 829`、`stitch/types.ts = 759`、`videoPromptLifecycle.ts = 490`、`runtimeSurfaceContent.ts = 407`、`videoSessionRuntime.ts = 512`、`videoSessionDraftController.ts = 309`、`videoScreenshotPreparationQueue.ts = 401`、`videoScreenshotPreparationRequestStore.ts = 294`、`videoScreenshotCacheRepository.ts = 420`、`videoScreenshotCacheIndexedDbStore.ts = 335`、`runtimeMessages.ts = 324`、`analyticsSchema.ts = 478`、`stitch/ui/components.ts = 592`、`yaml-config-editor/view.ts = 586`。`tools/report-performance-hotspots.mjs` 中的 line budgets 是当前 upper-bound hard gate；进一步收紧必须 standalone 通过后再同步。
...
- 当前业务/运行时/GA 重点实测：`videoSessionRuntime.ts = 512`、`videoSessionDraftController.ts = 309`、`videoScreenshotPreparationQueue.ts = 401`、`videoScreenshotPreparationRequestStore.ts = 294`、`videoScreenshotCacheRepository.ts = 420`、`videoScreenshotCacheIndexedDbStore.ts = 335`、`videoCaptureMutationTransaction.ts = 245`、`VideoDialogPanel.ts = 425`、`videoControlBarButton.ts = 299`、`sessionDraftRepository.ts = 399`、`runtimeMessages.ts = 324`、`bilibiliRichText.ts = 302`、`bilibiliPlatformObserver.ts = 286`、`markdownBuilder.ts = 288`、`PrivacySettingsView.ts = 255`、`yaml-config-editor/rowModel.ts = 254`、`analyticsSchema.ts = 478`、`eventCatalog.ts = 78`、`analyticsSanitizers.ts = 95`、`analyticsConfig.ts = 368`、`analyticsConfig.template.ts = 364`、`googleAnalyticsReporter.ts = 260`。
- P09 hybrid-cache support files 当前实测：`src/content/video/videoScreenshotCacheStore.ts = 97`、`src/content/video/videoScreenshotEncoding.ts = 152`、`src/content/video/videoSessionDraftScreenshotCache.ts = 216`、`src/background/services/videoScreenshotCacheIndexedDbStore.ts = 335`、`tests/e2e/utils/videoScreenshotCacheIndexedDb.ts = 220`。`videoScreenshotCacheIndexedDbStore.ts` 已进入 guarded hotspot，并由 `tools/report-performance-hotspots.mjs` 的 `<= 335` 预算约束；其余路径仍低于新增 hard budget 阈值。
