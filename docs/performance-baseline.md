...
2026-08-27 U04B Options section invalidation 重新采集 `audit:performance:report`：sourceFiles=`893`、hotspotsOver250=`101`、registeredLineBudgets=`145`。按报告的 newline 计数，新增 `sectionInvalidation.ts = 195` 行，低于 `250` 新 owner 上限；既有受控热点保持 `productionStitchRenderLifecycle.ts = 250 <= 253`、`productionStitchShellMount.ts = 248 <= 254`、`productionStitchActions.ts = 297 <= 302`、`productionStitchShellActionRuntime.ts = 356 <= 358`、`productionStitchPersistence.ts = 379 <= 379`。Fresh dev build 为 chunks=`111`、`runtimeEntry=327576 B <= 327680 B`，新增 lazy `sectionInvalidation` chunk=`4.9 KB`；production build 为 chunks=`96`、`runtimeEntry=161.3 KB`，release surface 为 `176` files 且 forbidden harness / pseudo-locale 均为 none。报告只保留 accepted-base 的 `optionsMutationCoordinator.ts` / `optionsStore.ts` missing budgets 与 onboarding bootstrap、runtimeMessages、localVaultPermissionFrame、usageStats 四项既有 overage；U04B 未修改 budget/gate 文件，也未放宽 build、chunk 或 line hard stop。
...
- `src/dev/contentOrchestratorHarness.ts`: `356` 行（hard budget 仍为 `<=359`）
