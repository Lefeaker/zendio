...
- `src/background/listeners/runtimeMessages.ts`: `331` 行
...
- 全部当前 `src` >250 LOC 文件均有 guarded line budget；2026-06-13 GA P01 schema core 后当前 sourceFiles=`758`、hotspotsOver250=`92`、registeredLineBudgets=`118`，预算以 `tools/report-performance-hotspots.mjs` 为准。
...
- 当前高信号热点实测：`stitch/content.ts = 867`、`messages.generated.ts = 829`、`stitch/types.ts = 759`、`videoPromptLifecycle.ts = 490`、`runtimeSurfaceContent.ts = 407`、`videoSessionRuntime.ts = 479`、`videoScreenshotPreparationQueue.ts = 404`、`videoScreenshotPreparationRequestStore.ts = 306`、`analyticsSchema.ts = 308`、`stitch/ui/components.ts = 592`、`yaml-config-editor/view.ts = 586`。`tools/report-performance-hotspots.mjs` 中的 line budgets 是当前 upper-bound hard gate；进一步收紧必须 standalone 通过后再同步。
...
- 当前业务/运行时/GA 重点实测：`videoSessionRuntime.ts = 479`、`videoScreenshotPreparationQueue.ts = 404`、`videoScreenshotPreparationRequestStore.ts = 306`、`VideoDialogPanel.ts = 425`、`videoControlBarButton.ts = 299`、`sessionDraftRepository.ts = 399`、`runtimeMessages.ts = 351`、`bilibiliRichText.ts = 302`、`bilibiliPlatformObserver.ts = 286`、`markdownBuilder.ts = 288`、`PrivacySettingsView.ts = 255`、`productionStitchShellMount.ts = 230`、`yaml-config-editor/rowModel.ts = 254`、`analyticsSchema.ts = 308`、`eventCatalog.ts = 78`、`analyticsSanitizers.ts = 95`、`analyticsConfig.ts = 369`、`analyticsConfig.template.ts = 364`、`googleAnalyticsReporter.ts = 317`。
