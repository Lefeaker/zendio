...
- 总 chunk 数：`114`
- `chunks/runtimeEntry-*.js`: `240.0 KB`
...
- `src/options/stitch/content.ts`: `900` 行
- `src/i18n/generated/messages.generated.ts`: `804` 行
- `src/options/stitch/types.ts`: `742` 行
- `src/content/video/videoSessionRuntime.ts`: `695` 行
- `src/content/video/sessionOperations.ts`: `597` 行
...
- `src/content/video/videoScreenshotPreparationQueue.ts`: `429` 行
...
- 全部当前 `src` >250 LOC 文件均有 guarded line budget；2026-06-08 Options i18n PR/main merge 复核后当前 trackedSourceFiles=`726`、hotspotsOver250=`95`、registeredLineBudgets=`113`，预算以 `tools/report-performance-hotspots.mjs` 为准。
- 2026-06-06 video screenshot attachment verification 已补齐 `src/shared/attachments/videoScreenshotAttachmentTemplates.ts <= 523` 与 `src/background/application/videoScreenshotAttachmentPlanner.ts <= 269`；2026-06-08 当前 performance coverage 见上一条。
- 当前 top line budgets：`stitch/content.ts <= 900`、`messages.generated.ts <= 804`、`stitch/types.ts <= 742`、`videoSessionRuntime.ts <= 695`、`video/sessionOperations.ts <= 597`、`stitch/ui/components.ts <= 592`、`yaml-config-editor/view.ts <= 586`。
- M12 current truth：`src/i18n/messages.ts` 已缩为 generated type shim，`src/i18n/schemaShellMessages.ts` 与手写 `src/i18n/locales/*.ts` 已删除；generated i18n 当前热点预算为 `messages.generated.ts <= 804` 与 `schemaMessages.generated.ts <= 397`，并继续由 `tools/report-performance-hotspots.mjs` 管理 generated source exact current-line budgets。
- 当前业务/运行时/GA 重点 budgets：`videoSessionRuntime.ts <= 695`、`videoScreenshotPreparationQueue.ts <= 429`、`VideoDialogPanel.ts <= 404`、`videoControlBarButton.ts <= 386`、`sessionDraftRepository.ts <= 394`、`runtimeMessages.ts <= 331`、`reader/sessionDrafts.ts <= 329`、`bilibiliRichText.ts <= 302`、`bilibiliPlatformObserver.ts <= 254`、`markdownBuilder.ts <= 288`、`PrivacySettingsView.ts <= 255`、`productionStitchShellMount.ts <= 230`、`yaml-config-editor/rowModel.ts <= 254`、`eventCatalog.ts <= 485`、`analyticsSanitizers.ts <= 455`、`analyticsConfig.ts <= 369`、`analyticsConfig.template.ts <= 364`、`googleAnalyticsReporter.ts <= 317`。
...
- `videoSessionRuntime` 当前为 `695` 行，`videoScreenshotPreparationQueue` 当前为 `429` 行；session-draft P08 final integration 已把 `video/sessionOperations`、`reader/session`、`reader/sessionOperations`、`ReaderDialogPanel`、`sessionDraftRepository`、`reader/sessionDrafts`、`runtimeMessages` 与 `sessionPlatformController` 纳入 exact current-line budget，作为后续拆分观察项。
