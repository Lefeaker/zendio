...
- 2026-08-27 U04B Options section invalidation 真值：`src/ui/stitch-runtime/render/sectionInvalidation.ts` 持有 11-scope 闭合集合、reentrant scope coalescing、focus/selection/scroll snapshot 与 dispose/late-completion no-op；Options feature caller 显式路由到 owner scope，只有 initial/external refresh/import/reload 的已枚举路径进入 `all-invariant-recovery`。Focused Options Vitest 当前为 `9` 文件 / `105` tests，Chromium `optionsIncrementalRender.browser.test.ts` 为 `1` test，完整 Options e2e shard为 `4` 文件 / `7` tests，完整 Chromium visual matrix 为 `156` passed / `6` skipped。UI ownership 为 `56 = 47 runtime + 9 compile`，dependency audit 为 `modules=1022 dependencies=3425 violations=0`；dev `runtimeEntry` 为 `327576 B <= 327680 B` hard stop，lazy `sectionInvalidation` chunk 为 `4.9 KB`。浏览器命令使用 locked local Playwright CLI 与 U04B 专属端口 `4181`，不使用 `npx` / `npm exec`
