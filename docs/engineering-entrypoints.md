...
- 2026-06-17 English uncatalogued-copy build/performance 真值：P07d surface-localization owner split 后，production fast `audit:build:report` 当前输出 `content/runtime.js` raw `49,991` bytes、`onboarding/index.js` raw `9,974` bytes、chunk count `100`；`audit:performance:report` 当前输出 `sourceFiles=802`、`hotspotsOver250=100`、`registeredLineBudgets=128`；`productionStitchLocalization.ts` 当前为 `370` 行，拆出的 `productionStitchSurfaceLocalization.ts` 为 `236` 行且低于 >250 LOC hotspot 阈值。完整当前热点与 line-budget 口径以 [`performance-baseline.md`](./performance-baseline.md)、`tools/report-build-splitting.mjs` 和 `tools/report-performance-hotspots.mjs` 为准，本次没有放宽 entry、single/shared chunk、locale chunk、YAML 或 line-budget hard gate。
