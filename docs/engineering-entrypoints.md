...
- `npm run lint:type-any`：扫描当前集成树；overall 为 `any: 0`、`unknown: 994`、assertions `1674`、non-null assertions `108`、`ts-expect-error: 5`。
...
- `npm run lint:type-any:ratchet`：同时守住 overall `0/994/1674/108/5`、src `0/553/623/5/0`、tests `0/441/1052/103/5`，并已接入 `quality` 作为 type-debt hard gate；tests 下降不得抵消 src 增长。
