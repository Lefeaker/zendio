...
Linux 的 `playwright-v1` child 固定使用 `LANG=C.UTF-8` / `LC_ALL=C.UTF-8`，以便 Chromium 的原生 Downloads 与 File System Access 正确处理中文等非 ASCII 文件名。其他工具及平台保留固定 `C` locale；调用方的 locale 值不能覆盖该策略。不要通过改写中文文件名来绕过测试进程的字符编码问题。
