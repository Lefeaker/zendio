...
- 2026-06-16 dependency-audit 真值：Node `v20.20.2` / npm `10.8.2` 下，P02 已将 dev toolchain targeted update 到 `vitest 3.2.6`、`@vitest/coverage-v8 3.2.6`、`web-ext 10.4.0` 与 root `esbuild 0.28.1`；本次又以 package-lock-only transitive update 将 dev-only `js-yaml` 从 `4.1.1` 升到 `4.2.0`。P09 后复跑曾发现 full dev audit 为 total `7`、severity `1 moderate / 6 high / 0 critical`，其中 moderate 是 `js-yaml <=4.1.1`，路径仅限 `eslint` / `@eslint/eslintrc`、`stylelint -> cosmiconfig`、`web-ext -> addons-linter -> eslint`，且 `npm ls js-yaml --omit=dev --all` 为 empty，因此不是 production release blocker。当前 `npm audit --omit=dev --json` 为 `0` vulnerabilities，production runtime release gate 保持 green；当前 `npm audit --audit-level=low --json` 仍退出 `1`，但已回到 `6` 个 `high` / `0` 个 `moderate` / `0` 个 `critical` / total `6`，涉及 `@vitest/coverage-v8`、`@vitest/mocker`、`esbuild`、`vite`、`vite-node` 与 `vitest`，且残留链仍位于 dev/build/test-only 路径：`@vitest/coverage-v8 -> vitest -> @vitest/mocker / vite-node -> vite 7.3.5 -> vite/node_modules/esbuild 0.27.7`。`npm audit` 的 fixAvailable 指向 semver-major `vitest@4.1.9`，因此 `0.2.0` 当前将剩余 high 链作为 release exception 处理，不得擅自跳到 Vitest 4 / Vite major；full dev audit 仍不是 `quality` / `verify:preflight` hard gate，不得表述为全量 dev audit green。后续 owner milestone 应单独执行 Vitest 4 / Vite compatibility migration。P01 type-ratchet 修复与 P02 toolchain update 合入本轮 integration 后，`build`、`build:firefox` 与 `package:firefox` 已重新通过；`package:firefox` 会生成 ignored local XPI 验证产物，不作为 source-of-truth 文件提交。
...
- `npm run lint:warnings-guard`：fresh run 通过；checked-in baseline 为 `160`，当前 warning 总数为 `160`，gate 输出为 `Warning 总量保持在基线 160 条`。
...
- `npm run lint:type-any`：fresh run 扫描 `1231` files；fresh overall 为 `any: 0`、`unknown: 1127`、assertions `1930`、non-null assertions `47`、`ts-expect-error: 3`；src 为 `0/621/677/9/0`；tests 为 `0/506/1253/38/3`。
...
- `npm run lint:type-any:ratchet`：checked-in 上限当前为 overall `0/1138/1934/53/4`、src `0/623/678/9/0`、tests `0/515/1256/46/4`，并已接入 `quality` 作为 type-debt hard gate；fresh run 通过，实测为 overall `0/1127/1930/47/3`、src `0/621/677/9/0`、tests `0/506/1253/38/3`。`verify:preflight` 当前不执行该 gate；本次没有放宽 `any`、`ts-expect-error`、warning baseline 或 checked-in type ratchet 上限。
