...
- 2026-05-29 Plan 10 D3 dependency-audit 真值：Node `v20.20.2` / npm `10.8.2` 下，`npm audit --omit=dev` 为 `0` vulnerabilities，production runtime release gate 仍为 green；`web-ext` batch 后 `npm audit --audit-level=low` 仍为 dev/release toolchain 后续计划项，当前报告为 `8` vulnerabilities（`3` moderate / `5` high），涉及 `ajv`、`brace-expansion`、`fast-uri`、`lodash`、`minimatch`、`node-forge`、`picomatch`、`ws`
