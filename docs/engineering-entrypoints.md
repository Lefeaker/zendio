...
- `npm run quality`
  ...
  - 显式包含 `audit:i18n-uncatalogued-user-copy:check`；英文 uncatalogued-copy audit 已作为 hard gate 接入 `quality`，覆盖 `normalizeToAppError` 的 raw English `defaultMessage` fallback、`subtitle` / `hint` / `body` 等 production-visible fields，并且不会把带编号的多词英文 UI title 误判为 technical token list；当前输出 `scanned=575 findings=0 unexpected=0 staleAllowlist=0`，allowlist rules 为 `0`
