...
│   ├── shared/            # 共享模块
│   ├── i18n/              # 国际化系统
│   │   ├── catalog/messages/ # catalog source: runtime/static/schema
│   │   ├── generated/     # catalog generator output
│   │   └── runtime/       # runtime locale loading and page binding
│   ├── styles/            # 样式文件
│   ├── third_party/       # 第三方集成
│   └── types/             # 类型定义
├── public/                # 静态资源（版本控制）
│   ├── icons/             # 扩展图标
│   ├── _locales/          # generated WebExtension static messages
│   └── manifest.json      # 扩展清单
├── build/                 # 构建产物（忽略版本控制）
│   ├── dist/              # 构建输出
│   ├── releases/          # 发布包
...
public/_locales/
└── zh_CN/messages.json # generated by npm run i18n:catalog:generate
```

`public/_locales/**` 是 catalog generator 的 checked-in WebExtension static output，不手写。新增或修改文案时只改 `src/i18n/catalog/messages/<lang>/runtime.json`、`schema.json` 或 `static.json`。
...
const localeSourcePath = 'src/i18n/catalog/messages/zh-CN/runtime.json';
...
| `src/i18n/`                   | 国际化          | `catalog/messages/`, `generated/`, `runtime/`    |
| `public/`                     | 静态资源        | `icons/`, generated `_locales/`, `manifest.json` |
...
| 本地化文件 | `'src/i18n/catalog/messages/zh-CN/runtime.json'`  |
...
1. 修改 catalog source：
   - 运行时 UI/业务文案：`src/i18n/catalog/messages/<lang>/runtime.json`
   - Options schema 文案：`src/i18n/catalog/messages/<lang>/schema.json`
   - Manifest / WebExtension static 文案：`src/i18n/catalog/messages/<lang>/static.json`

```json
// src/i18n/catalog/messages/en/runtime.json
{
  "bookmarkSaved": "Bookmark saved successfully",
  "bookmarkFailed": "Failed to save bookmark: {error}"
}
```

```json
// src/i18n/catalog/messages/zh-CN/runtime.json
{
  "bookmarkSaved": "书签保存成功",
  "bookmarkFailed": "保存书签失败：{error}"
}
```

2. 生成并校验 catalog artifacts：
