...
export function collectChineseFamilyMessageContainers(
  ...
  for (const candidate of collectOrderedElements(root, selectors)) {
    if (options.shouldSkip?.(candidate)) {
      continue;
    }
    if (!normalizeText(candidate.textContent ?? '')) {
      continue;
    }
    if (containers.some((container) => container.contains(candidate))) {
      continue;
    }
    if (containers.some((container) => candidate.contains(container))) {
      continue;
    }
...
export function resolveChineseFamilyRoleFromToken(
  ...
  if (/user|human|prompt|question|send/.test(normalized)) {
    return 'user';
  }
  if (/assistant|bot|answer|deepseek|doubao|qwen/.test(normalized)) {
    return 'assistant';
  }
...
export { doubaoParser } from './doubao';
export { kimiParser } from './kimi';
export { tongyiParser } from './tongyi';
