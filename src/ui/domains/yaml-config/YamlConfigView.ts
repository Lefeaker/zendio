...
export class YamlConfigView extends BaseComponent<YamlConfigViewRenderContext> {
  ...
  private mountLayout(onDirty: () => void): void {
    ...
    this.tableHost.dataset.namePlaceholder =
      this.messages?.yamlFieldCustomNamePlaceholder ?? 'Field name';
    this.tableHost.dataset.placeholderValue =
      this.messages?.yamlFieldDefaultPlaceholder ?? 'Field value';
    this.tableHost.dataset.placeholderArray = normalizeArrayPlaceholderText(
      extMessages?.yamlFieldArrayPlaceholder
    );
    this.tableHost.dataset.hintArray = normalizeArrayHintText(extMessages?.yamlFieldArrayHint);
    this.tableHost.dataset.hintArrayPreviewEmpty =
      extMessages?.yamlFieldArrayPreviewEmpty ?? '未配置数组项';
    this.tableHost.dataset.labelAdvancedShow =
      this.messages?.yamlFieldAdvancedShowLabel ?? 'Show source';
    this.tableHost.dataset.labelAdvancedHide =
      this.messages?.yamlFieldAdvancedHideLabel ?? 'Hide source';
    this.tableHost.dataset.labelValuePath = this.messages?.yamlFieldValuePathLabel ?? 'Value path';
    this.tableHost.dataset.placeholderValuePath =
      this.messages?.yamlFieldValuePathPlaceholder ?? 'e.g. meta.author or extra.notes[0]';
    this.tableHost.dataset.hintValuePath =
    ...
    this.tableHost.dataset.labelValuePathExamplesTitle =
      extMessages?.yamlFieldValuePathExamplesTitle ?? '常见上下文字段';
    this.tableHost.dataset.hintValuePathExamples =
    ...
    this.tableHost.dataset.labelCustomGroup =
      this.messages?.yamlCustomGroupLabel ?? 'Custom fields';
    this.tableHost.dataset.errorNameRequired =
      this.messages?.yamlFieldErrorNameRequired ?? 'Field name is required';
    this.tableHost.dataset.errorNamePattern =
    ...
    this.domainHost.dataset.labelHint =
    ...
    this.domainHost.dataset.placeholderDomain =
      extMessages?.yamlDomainPlaceholder ?? '例如 example.com 或 *.example.com';
    this.domainHost.dataset.labelContentType =
      extMessages?.yamlDomainContentTypeLabel ?? '内容类型';
    ...
    this.domainHost.dataset.placeholderValue =
      extMessages?.yamlDomainFieldValuePlaceholder ?? '默认值（可选）';
    this.domainHost.dataset.placeholderValueArray = normalizeArrayPlaceholderText(
      extMessages?.yamlDomainFieldArrayPlaceholder
    );
    this.domainHost.dataset.hintValueArray = normalizeArrayHintText(
      extMessages?.yamlDomainFieldArrayHint
    );
    this.domainHost.dataset.hintArrayPreviewEmpty =
      extMessages?.yamlDomainFieldArrayPreviewEmpty ?? '未配置数组项';
    this.domainHost.dataset.labelValuePath =
      extMessages?.yamlDomainValuePathLabel ?? 'Value path (可选)';
    this.domainHost.dataset.placeholderValuePath =
      extMessages?.yamlDomainValuePathPlaceholder ?? '如 meta.author';
    this.domainHost.dataset.errorDomainRequired =
      extMessages?.yamlDomainErrorDomainRequired ?? '域名不能为空。';
    this.domainHost.dataset.errorDomainDuplicate =
      extMessages?.yamlDomainErrorDomainDuplicate ?? '同一内容类型中存在重复的域名规则。';
    this.domainHost.dataset.errorFieldRequired =
      extMessages?.yamlDomainErrorFieldRequired ?? '至少选择一个字段。';
    this.domainHost.dataset.errorFieldDuplicate =
      extMessages?.yamlDomainErrorFieldDuplicate ?? '同一规则中存在重复字段。';
    this.domainHost.dataset.errorFieldUnsupported =
      extMessages?.yamlDomainErrorFieldUnsupported ?? '字段在当前内容类型中不可用：';
    this.domainHost.dataset.errorValueInvalid =
      extMessages?.yamlDomainErrorValueInvalid ?? '字段默认值与类型不匹配：';
    this.domainHost.dataset.errorValuePathInvalid =
      extMessages?.yamlDomainErrorValuePathInvalid ?? 'Value path 不能包含空格。';
    this.domainHost.dataset.warningUnresolved =
