...
import { UiInput } from '../../primitives/input';
import { UiSelect } from '../../primitives/select';
...
export class VaultRouterView extends BaseComponent<VaultRouterViewConfig> {
  ...
  render(config: VaultRouterViewConfig): HTMLElement {
    ...
    this.rowsHost = this.createElement('div', 'divide-y divide-border/50', {
      'data-role': 'routing-rows'
    });
    this.emptyHint = this.createElement('div', 'p-8 text-center text-base-content/60 italic', {
      'data-role': 'routing-empty'
    });
    ...
    const controls = this.createElement('div', 'flex justify-end mt-4', {
      'data-role': 'routing-controls'
    });
    const addRuleButtonHost = this.createElement('div');
    new UiButton(addRuleButtonHost).render({
  ...
  update(state: VaultRouterViewState): void {
    ...
    this.rowsHost.replaceChildren(
      ...rules.map((rule) => this.buildRow(rule, vaultOptions, defaultVaultId))
    );
  ...
  private buildHeader(labels: VaultRouterViewLabels): HTMLElement {
    const headerRow = this.createElement(
      ...
      [
        GRID_CLASS,
        'p-3 bg-base-200 border-b border-base-300 font-medium text-base-content/60'
      ].join(' '),
  ...
  private buildRow(
    ...
    const enabledCheckbox = new UiCheckbox(enabledHost).render({
      checked: rule.enabled ?? false,
      ariaLabel: this.config.labels.enabledAria,
      labelClassName: 'justify-center',
      inputClassName: 'routing-rule-enabled'
    });
    enabledCheckbox.checked = rule.enabled ?? false;
    enabledCheckbox.addEventListener('change', () => {
      this.config?.onToggleEnabled(rule.id, enabledCheckbox.checked);
    });
    enabledCell.append(enabledHost);
    ...
    const typeHost = this.createElement('div', 'w-full');
    const typeSelect = new UiSelect(typeHost).render({
      ariaLabel: this.config.labels.typeAria,
      value: rule.type,
      className: 'h-8 text-sm routing-rule-type min-h-[32px]',
      options: typeLabels.map(({ type, label }) => ({ value: type, label }))
    });
    ...
    const patternInput = new UiInput(patternHost).render({
      type: 'text',
    ...
    const targetHost = this.createElement('div', 'w-full');
    const targetSelect = new UiSelect(targetHost).render({
      ariaLabel: this.config.labels.targetAria,
      className: 'h-8 text-sm routing-rule-target min-h-[32px]',
      options: []
    });
    this.populateTargetOptions(
      targetSelect,
      vaultOptions,
      rule.vaultId ?? defaultVaultId ?? vaultOptions[0]?.id ?? ''
    );
    ...
    const priorityInput = new UiInput(priorityHost).render({
      type: 'number',
    ...
    new UiButton(removeButtonHost).render({
