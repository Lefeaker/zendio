...
- `utils/`：辅助方法（导入导出、transfer 等）；`styles/` 目录承载 Tailwind 输入与必要的外部样式（如 `obsidian-clipper-style.css`）。

### 0.2 样式规范速览
- 样式入口固定为 `src/options/styles/tailwind.css` 与 `src/styles/global.tailwind.css` 的静态产物链路；所有 DOM 类名继续使用 `.aobx-*` 前缀。
...
#### DaisyUI 组件工厂函数 (Phase 1 迁移)
| 名称 | 用途 | 示例 | 备注 |
| --- | --- | --- | --- |
| `createButton()` | 创建 DaisyUI 按钮 | `createButton('保存', { variant: 'primary', size: 'sm' })` | 支持 variant、size、shape、loading 配置 |
| `createInput()` | 创建 DaisyUI 输入框 | `createInput({ type: 'email', placeholder: '输入邮箱' })` | 支持 bordered、ghost、size 配置 |
| `createAlert()` | 创建 DaisyUI 提示框 | `createAlert('操作成功', { type: 'success' })` | 支持 info、success、warning、error 类型 |
| `AobFormGroup` | DaisyUI Card 包装器 | `new AobFormGroup(container).render({ label: '标题', control: input })` | 已迁移到 `.card` 结构，向后兼容 |

> **参考**: `src/options/components/shared/DaisyUIHelpers.ts`, `docs/251126-design-system-poc/PHASE1-MIGRATION-GUIDE.md`
...
- **唯一样式入口**：Options 页主要依赖 `src/options/styles/tailwind.css` 与 `src/styles/global.tailwind.css` 的构建产物，`design-tokens.css` 仅提供 token 值。
...
- **定期复查**：Options 模块维护人（默认由当期版本负责人承担）需在每季度迭代结束后检查 README 是否覆盖最新规范，并把复查记录写入 `docs/options-doc-refresh-log.md`。
