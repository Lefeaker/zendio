...
const RESULT_TYPE_CLASSES = {
  ...
  success: 'aobx-connection-result rounded-md border border-[color:color-mix(in_srgb,var(--aobx-status-success)_65%,var(--aobx-border))] bg-[color:color-mix(in_srgb,var(--aobx-status-success)_18%,transparent)] p-3 text-sm text-[color:color-mix(in_srgb,var(--aobx-status-success)_80%,black)] flex gap-2 leading-relaxed items-start',
  error: 'aobx-connection-result rounded-md border border-[color:color-mix(in_srgb,var(--aobx-status-error)_70%,var(--aobx-border))] bg-[color:color-mix(in_srgb,var(--aobx-status-error)_22%,transparent)] p-3 text-sm text-[color:color-mix(in_srgb,var(--aobx-status-error)_80%,black)] flex gap-2 leading-relaxed items-start'
} as const;
...
export async function runConnectionTest(
  ...
  try {
    ...
    const effectiveReason =
      detail === msgs.connectionFailed && testError?.message
        ? testError.message
        : detail;
...
export function renderConnectionResult(response: ConnectionTestResult, msgs: Messages): string {
  if (response.success) {
    ...
    const hasAdditionalPayload = typeof response.status !== 'undefined' || typeof response.response !== 'undefined';
...
function buildFailureHints(reason: string, status: number | undefined, msgs: Messages): string {
  ...
  if (/failed to fetch|networkerror|timeout|refused|reset|unreachable|无法连接|未配置可用的地址|未配置 api key/i.test(lower)) {
