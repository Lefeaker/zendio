...
import { queryAllFormControlElements, queryHTMLElement, queryFormControlElement } from '@shared/guards';
...
export interface ListEditor {
  /** 添加新行 */
  addRow(descriptor: ListRowDescriptor, options?: { autoFocus?: boolean; skipEmptyLimit?: boolean; }): HTMLElement;
  /** 移除行 */
...
export function createListEditor(config: ListEditorConfig): ListEditor {
  const resolvedContainer = typeof config.container === 'string'
  ...
  function addRow(descriptor: ListRowDescriptor, options: { autoFocus?: boolean; skipEmptyLimit?: boolean; } = {}): HTMLElement {
    // 检查空行限制
    ...
    descriptor.fields.forEach(field => {
      ...
      if (field.placeholder) {
        bindings.push(bindLocalizedAttr(fieldElement, 'placeholder', toLocalizedContent(field.placeholder)));
      }
    });
