...
export class Navigation extends BaseComponent<NavigationConfig> {
  ...
  render(config: NavigationConfig): HTMLElement {
    ...
    const ariaLabel =
      this.messages?.settingsTitle ??
      this.messages?.extensionSubtitle ??
      '设置导航';
    ...
    for (const item of config.items) {
      ...
      const link = this.createElement('a', 'aobx-navigation__link aobx-nav__link flex items-center gap-2 px-2 py-1.5 rounded-inherit text-md cursor-pointer no-underline leading-snug text-base-content hover:text-accent');
  ...
  private highlightActive(sectionId: string): void {
    const items = this.container.querySelectorAll<HTMLElement>('.aobx-navigation__item, .aobx-tree-item');
    items.forEach((item) => {
      const link = item.querySelector<HTMLAnchorElement>('a');
      if (item.dataset.sectionId === sectionId) {
        item.classList.add('is-current', 'is-active', 'aobx-navigation__item--active', 'bg-accent/12');
        link?.setAttribute('aria-current', 'true');
      } else {
        item.classList.remove('is-current', 'is-active', 'aobx-navigation__item--active', 'bg-accent/12');
