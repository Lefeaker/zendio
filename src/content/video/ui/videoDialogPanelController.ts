...
import {
  bindVideoDialogPanelEvents,
  type VideoDialogPanelEventHandlers
} from './videoDialogPanelEvents';

interface VideoItemTemplate {
  id: string;
  element: HTMLElement;
}

export interface VideoDialogPanelControllerOptions {
  host: HTMLElement;
  shadow: ShadowRoot;
  ...
  isCollapsed(): boolean;
  events: VideoDialogPanelEventHandlers;
}

export class VideoDialogPanelController {
  ...
  constructor(private readonly options: VideoDialogPanelControllerOptions) {
    ...
    this.items = createKeyedSessionList({
      ...
      before: this.handle.itemList.querySelector('.session-add-capture-card'),
  ...
  update(): void {
    ...
    const nextList = requireItemList(nextRoot);
    this.items.reconcile(collectItems(nextList));
...
function requireItemList(root: ParentNode): HTMLElement {
  ...
  if (!list) throw new Error('Video session surface is missing the item list');
  return list;
}

function collectItems(list: HTMLElement): VideoItemTemplate[] {
  return Array.from(list.children).flatMap((element) => {
    if (!(element instanceof HTMLElement)) return [];
    const id = element.dataset.captureId;
    return id ? [{ id, element }] : [];
  });
}

function patchAddCaptureCard(currentList: HTMLElement, nextList: HTMLElement): void {
  const current = currentList.querySelector<HTMLElement>('.session-add-capture-card');
  const next = nextList.querySelector<HTMLElement>('.session-add-capture-card');
  if (current && next) patchSessionElement(current, next);
}
