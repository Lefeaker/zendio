import { isObjectRecord, type ObjectRecord, type RuntimePropertyValue } from '../../shared/guards/object';
import {
...
type VideoScreenshotCacheIndexedDbIndex = {
  getAll(query?: IDBValidKey | IDBKeyRange | null): VideoScreenshotCacheIndexedDbRequest<
...
export function createVideoScreenshotCacheIndexedDbStore(
  ...
  const readAllEntries = (store: VideoScreenshotCacheIndexedDbObjectStore) =>
    requestToRecordArray(store, 'Failed to read video screenshot cache blob rows.').then(collectEntries);
