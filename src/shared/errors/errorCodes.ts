...
const isErrorCategoryValue = (
  value: string
): value is (typeof ERROR_CATEGORIES)[keyof typeof ERROR_CATEGORIES] => ERROR_CATEGORY_VALUES.has(value);
...
export function parseErrorCode(code: string): ParsedErrorCode {
  ...
  // 查找对应的域
  const domain = Object.entries(ERROR_CODE_PREFIXES)
    .find(([_, p]) => p === prefix)?.[0] as ErrorDomain || 'unknown';
