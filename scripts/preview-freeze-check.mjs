  ...
  [
    'original reference index.js vs current generated preview index.js',
    {
      reason: [
        ...
        'The O01 raw-first loss-aware Options codec, canonical strict taxonomy, persisted-empty REST credential boundary, and typed taxonomy-editor validation refresh the generated preview JS while preserving the frozen reference and Stitch DOM/visual/runtime contract. U02A refreshes the generated preview JS after the runtime-surface kernel moved the preview fixture onto the public Stitch runtime barrel, adding the concrete runtime renderer and surface graph while preserving the frozen reference and Stitch DOM/visual/runtime contract. U02B refreshes the generated preview JS after routing the Options surface primitives through their canonical semantic owners while preserving the frozen reference and Stitch DOM/visual/runtime contract. U02C1 refreshes the generated preview JS after promoting the neutral usage-chart owner and deleting the private Stitch renderer while preserving the frozen reference and Stitch DOM/visual/runtime contract. O03 refreshes the generated preview JS after deriving Options and taxonomy types from their canonical schemas and hardening the schema-owned runtime boundaries while preserving the frozen reference and Stitch DOM/visual/runtime contract.',
        'The 2026-09-06 Options remediation preview sync incorporates accepted 5d2f505c session-panel incremental runtime/schema inputs and 16b9ed60 Clipper live destination-selection builder inputs. Two independent public preview builds from reviewed integration b9d55024, using the unchanged preview fixture entries, build-preview owner, package lock and Node 20.20.2, produced identical complete artifacts and the exact JS digest below while preserving the original reference and strict comparison contract.',
        'Milestone A refreshes the generated preview JS hash after adding catalog-backed auto-save quota guidance while reusing the existing localized saveFailed and saveButton messages, preserving the frozen reference and Stitch DOM, visual, and runtime contract.',
        'The accepted B1 integration after the Milestone A digest added preview-reachable vault identity validation through src/shared/config/vaultRouterIdentity.ts and src/shared/schemas/vault.schema.ts. This B1 catalog correction refreshes the generated preview JS hash after replacing validator-authored English issue prose with the admitted structured code/path/typed-values contract and switching Zod emission to the technical vault-router-identity:<issue-code> form, while preserving the frozen reference and Stitch DOM, visual, and runtime contract.',
        'Milestone B2 refreshes the generated preview JS hash after adding the explicit Maintenance idle, running, success, failure, and source-notice schema projection with its five-for-five catalog/generated-key replacement, plus truthful aria-current=page output for the active settings and modal-resource links in the shared initial shell builders, while preserving the frozen reference and Stitch DOM, visual, and runtime contract.',
        'The content locale/focus correction refreshes the generated preview JS hash after the preview-reachable shared Video surface switches only its fragment-note translation request from the timestamp placeholder key to the existing fragment-compatible reader highlight placeholder key. Two independent Node 20.20.2 builds produced identical complete artifacts; generated index/onboarding HTML and styles remained unchanged, while only JavaScript bundles and their embedded standalone HTML changed, preserving the frozen reference, comparison logic, tolerances, and Stitch DOM/visual/runtime contract.'
      ].join(' '),
      leftSha256: '9020ccbd91acd691eccd3fdf568b9a90efbddf0a35d79f36ef1caba702fa0c07',
      // U02A runtime-surface kernel; U02B primitive/token parity; U02C1 usage-chart owner; O03 canonical-schema refresh.
      rightSha256: '15b6b527a0855d6bdf1426c09385f18ebe0cec5e3d667c65ed1484325a326652'
    }
  ]
]);
