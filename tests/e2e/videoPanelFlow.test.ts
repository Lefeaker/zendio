...
async function installExportCaptureListener(extensionPage: Page, testInfo: TestInfo): Promise<void> {
  await runStage(testInfo, 'install export capture listener', async () => {
    await extensionPage.evaluate(() => {
      ...
      const captureState =
        runtimeWindow.__aiobP07VideoClipCapture ??=
          { installed: false, lastVideoClip: null };
      ...
      if (!captureState.installed) {
        chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
          if (
            ...
            captureState.lastVideoClip =
              ((message as { data?: Record<string, unknown> }).data ?? null) as
...
async function readVideoDraftEntries(extensionPage: Page): Promise<VideoDraftEntry[]> {
  return extensionPage.evaluate(async ({ storageKeyPrefix, indexKey }) => {
    const storage = await chrome.storage.local.get(null);
      ...
      .map(([key, value]) => {
        ...
        const envelope =
          typeof value === 'object' && value !== null
            ...
            : {};
        const captures = Array.isArray(envelope.payload?.captures) ? envelope.payload.captures : [];
  ...
  testWithExtension(
    ...
    async ({ context, extensionPage }, testInfo) => {
      ...
      try {
          ...
          .poll(
            async () => {
              ...
              return {
                ...
                containsScreenshotDataUrl: entries.some((entry) => entry.json.includes('data:image'))
              };
            },
            {
              timeout: 10000,
        ...
        await expect(page.locator('[data-role="capture-item"]')).toHaveCount(2);
        await expect(
          page.locator('[data-role="capture-item"]').first().locator('[data-action-id="video:toggle-screenshot"]')
        ...
        await expect
          ...
          .not.toBeNull();
        const resolvedClip = (await readCapturedVideoClip(extensionPage)) as NonNullable<
          ExportedVideoClip
        >;
