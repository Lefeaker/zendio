...
async function simulateTransientVisibilityRoundTrip(
  page: Page,
  testInfo: TestInfo
): Promise<void> {
...
async function installReaderExportCaptureListener(
  ...
  await runStage(testInfo, 'install reader export capture listener', async () => {
    await extensionPage.evaluate(() => {
      ...
      const captureState =
        runtimeWindow.__aiobP07ReaderClipCapture ??=
          { installed: false, lastClipResult: null };
      ...
      if (!captureState.installed) {
        chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
          if (
            ...
            captureState.lastClipResult =
              ((message as { payload?: ReaderClipPayload }).payload ?? null) as ReaderClipPayload;
...
async function openReaderFromSelection(
  ...
  const readerAction = page.locator('[data-stitch-surface="clipper"] button[data-action-id="reader"]');
...
testWithExtension.describe('Reader Panel E2E Flow', () => {
  ...
  testWithExtension(
    ...
    async ({ page, context, extensionPage }, testInfo) => {
      ...
      const firstInput = page.locator('[data-stitch-surface="reader"] input[data-highlight-input]').first();
      ...
      await openReaderFromSelection(
        ...
        await expect(page.locator('[data-stitch-surface="reader"] article[data-highlight-id]')).toHaveCount(
        2
      );
      await expect
        .poll(
