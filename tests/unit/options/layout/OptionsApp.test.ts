...
const contentMountSectionMock = vi.hoisted(() => vi.fn(() => Promise.resolve({ id: 'section-instance' })));
...
describe('OptionsApp', () => {
  ...
  it('renders sidebar and content with default brand and footer links from messages', () => {
    ...
    expect(sidebarConfig?.footerLinks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: 'Open onboarding', href: '../onboarding/index.html', external: true }),
  ...
  it('configureUI replaces the prior modal controller instance', () => {
    app.render({ stateManager, formRegistry, modalBindings: [{ triggerId: 'supportLink', modalId: 'supportModal' }] });
