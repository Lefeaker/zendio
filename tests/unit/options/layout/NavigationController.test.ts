...
class MockIntersectionObserver implements IntersectionObserver {
  ...
  emit(target: Element, isIntersecting: boolean = true, intersectionRatio: number = isIntersecting ? 1 : 0): void {
  ...
  emitEntries(entries: Array<{ target: Element; isIntersecting?: boolean; intersectionRatio?: number }>): void {
    this.callback(entries.map(({ target, isIntersecting = true, intersectionRatio = isIntersecting ? 1 : 0 }) => {
      const rect = target.getBoundingClientRect();
      return {
        time: Date.now(),
        target,
...
const originalScrollIntoViewDescriptor = Object.getOwnPropertyDescriptor(
  ...
  afterEach(() => {
    ...
    document.body.innerHTML = '';
    if (originalObserver) {
      (globalThis as { IntersectionObserver: typeof IntersectionObserver }).IntersectionObserver = originalObserver;
    } else {
  ...
  it('constructs as no-op when navigation container or observer factory is missing', () => {
    ...
    document.body.innerHTML = '<nav class="aobx-navigation"></nav><main><section id="section-usage"></section></main>';
    const noObserverController = new NavigationController({ document, window, observerFactory: null as unknown as typeof IntersectionObserver });
  ...
  it('picks the highest-ratio visible entry and reacts to section mount lifecycle', () => {
    ...
    if (!(usageSection instanceof HTMLElement) || !(restSection instanceof HTMLElement) || !instance) {
      throw new Error('Required navigation elements missing');
    }

    restSection.dataset.sectionMounted = 'true';
    document.dispatchEvent(new CustomEvent('aob:sectionmounted', { detail: { sectionId: 'rest' } }));
    ...
    document.dispatchEvent(new CustomEvent('aob:sectionunmounted', { detail: { sectionId: 'rest' } }));
  ...
  it('uses auto scroll when reduced motion is preferred and keeps preset active item without hash', () => {
    ...
    const matchMediaSpy = vi.spyOn(window, 'matchMedia').mockImplementation(() => ({ matches: true } as MediaQueryList));
    const restSection = document.getElementById('section-rest');
