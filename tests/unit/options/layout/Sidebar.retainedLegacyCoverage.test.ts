...
describe('Sidebar', () => {
  ...
  it('renders brand details with logo and version tag', () => {
    ...
    expect(container.querySelector('.aobx-sidebar__brand-title')?.textContent).toBe(
      'All in Obsidian'
    );
  ...
  it('uses translated default navigation labels when messages are available', () => {
    ...
    const links = Array.from(container.querySelectorAll('.aobx-navigation__link')).map(
  ...
  it('forwards active navigation updates to the child navigation component', () => {
    ...
    const navigation = (sidebar as unknown as { navigation: { setActive: (id: string) => void } })
      .navigation;
