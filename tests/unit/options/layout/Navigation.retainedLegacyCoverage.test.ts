...
describe('Navigation', () => {
  ...
  it('updates the active section and invokes onNavigate when clicking a new item', () => {
    ...
    expect(
      container.querySelector('[data-section-id="rest"]')?.classList.contains('is-active')
    ).toBe(true);
    expect(
      container.querySelector('[data-section-id="rest"] a')?.getAttribute('aria-current')
    ).toBe('true');
    expect(
      container.querySelector('[data-section-id="usage"] a')?.hasAttribute('aria-current')
    ).toBe(false);
  });
  ...
  it('setActive updates classes after render', () => {
    ...
    expect(
      container
        .querySelector('[data-section-id="diagnosis"]')
        ?.classList.contains('aobx-navigation__item--active')
    ).toBe(true);
    expect(
      container.querySelector('[data-section-id="diagnosis"] a')?.getAttribute('aria-current')
    ).toBe('true');
    expect(
      container.querySelector('[data-section-id="usage"]')?.classList.contains('is-active')
    ).toBe(false);
  });
});
