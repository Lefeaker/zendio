...
const stateManagerSetStateMock = vi.hoisted(() => vi.fn());
...
describe('mountOptionsShell', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    document.body.innerHTML = '<div id="optionsShellRoot" hidden></div>';
    window.history.replaceState({}, '', '/options.html#section-rest');
  ...
  it('mounts the shell, binds options and usage state, and exposes runtime controls', async () => {
    const { mountOptionsShell } = await import('../../../src/options/app/optionsShell');
    const mounted = await mountOptionsShell({} as never);
  ...
  it('mountAllSections skips the mounted initial section and cleanup tears everything down', async () => {
    const { mountOptionsShell } = await import('../../../src/options/app/optionsShell');
    const mounted = await mountOptionsShell({} as never);
  ...
  it('throws when the shell container is missing', async () => {
    document.body.innerHTML = '';
    const { mountOptionsShell } = await import('../../../src/options/app/optionsShell');

    await expect(mountOptionsShell({} as never)).rejects.toThrow(
      '[OptionsShell] Required #optionsShellRoot container is missing.'
    );
  });
});
