...
import {
  preprocessDocument,
  preprocessDocumentFull,
  removeAdsAndTracking,
  cleanupAttributes
} from '../../../src/third_party/obsidian-clipper/domPrep';
...
describe('domPrep', () => {
  it('removes script-like elements and absolutizes urls', () => {
    const doc = createDocument(
      '<html><body><script></script><img src="/img.png"><a href="/page">Link</a><img srcset="/a.png 1x, /b.png 2x" data-src="/lazy.png"></body></html>'
    );
    ...
    expect(doc.querySelector('[srcset]')?.getAttribute('srcset')).toContain(
      'https://example.com/a.png 1x'
    );
    expect(doc.querySelector('[data-src]')?.getAttribute('data-src')).toBe(
  ...
  it('cleans ad selectors, removable attributes, and tolerates invalid selectors', () => {
    ...
    const doc = createDocument(
      '<html><body><div class="ad">ad</div><button onclick="alert(1)" data-track="x">ok</button></body></html>'
    );
