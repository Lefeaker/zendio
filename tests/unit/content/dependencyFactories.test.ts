...
const createAIChatExtractorMock = vi.hoisted(() => vi.fn(() => ({
  id: 'ai.chat',
  priority: 200,
...
vi.mock('../../../src/platform', async () => {
  const actual = await vi.importActual<typeof import('../../../src/platform')>('../../../src/platform');
  return {
    ...actual,
...
describe('content dependency factories', () => {
  ...
  it('does not call getPlatformServices inside reader session factory', () => {
    const platform = {
      optionsRepository: { get: vi.fn().mockResolvedValue({}), set: vi.fn(), onChange: vi.fn(() => () => undefined) },
  ...
  it('does not call getPlatformServices inside video prompt factory', () => {
    const platform = { storage: { sync: {}, local: {}, session: {} }, runtime: { getURL: vi.fn(), openOptionsPage: vi.fn(), onInstalled: vi.fn(), onStartup: vi.fn() } };
