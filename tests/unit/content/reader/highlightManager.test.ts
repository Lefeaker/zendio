...
describe('ReaderHighlightManager', () => {
  ...
  beforeEach(() => {
    ...
    document.documentElement.removeAttribute('data-aiobReaderHighlight');
    document.documentElement.removeAttribute('data-aiobReaderHighlightTheme');
    manager = new ReaderHighlightManager(document);
  });

  it('applies highlight theme onto document dataset', () => {
    manager.applyTheme('gradient');
    expect(document.documentElement.dataset.aiobReaderHighlight).toBe('gradient');
    expect(document.documentElement.dataset.aiobReaderHighlightTheme).toBe('gradient');
  ...
  it('sorts highlights by document order and resolves primary wrapper from connected segment', () => {
    document.body.innerHTML = '<article><p id="first">First block</p><p id="second">Second block</p></article>';
  ...
  it('unwraps highlight segments and reconstructs multi-segment text', () => {
    document.body.innerHTML = '<article><p><span id="a">Hello</span><span id="b">World</span></p></article>';
...
it('merges inline highlight segments within the same block into a single wrapper', () => {
  const manager = new ReaderHighlightManager(document);
  document.body.innerHTML = '<article><p id="content"><span id="a">Hello</span><span id="b">World</span></p></article>';
...
it('reconstructs with ascii spacing, preserves cjk adjacency, and returns longer detached text', () => {
  ...
  expect(manager.reconstructText({
    id: 'ascii',
    selectedHtml: '<p>Hello world</p>',
    selectedText: 'Hello world',
    comment: '',
...
it('returns null when wrappers and fallback fragments both stay empty, and keeps nested detached marks safe', () => {
  ...
  const wrappers = (manager as unknown as { createHighlightWrappers: (range: Range, id: string) => HTMLElement[] }).createHighlightWrappers(range, 'blank');
  ...
  nested.remove();
  expect(() => (manager as unknown as { flattenNestedHighlightMarks: (wrapper: HTMLElement) => void }).flattenNestedHighlightMarks(wrapper)).not.toThrow();
});

it('keeps split segments when merge extract fails and assigns metadata roles across segments', () => {
  document.body.innerHTML = '<article><p><span id="a">One</span><span id="b">Two</span><span id="c">Three</span></p></article>';
  ...
  const merged = (manager as unknown as { mergeWrapperSegments: (segments: HTMLElement[], id: string) => HTMLElement[] }).mergeWrapperSegments([a, b, c], 'merge-id');
...
it('returns null when wrapper creation falls through select, surround, and empty extract paths', () => {
  ...
  const extractSpy = vi.spyOn(Range.prototype, 'extractContents').mockImplementationOnce(() => document.createDocumentFragment());
...
it('keeps sort stable when two highlights share the same wrapper', () => {
  ...
  document.body.appendChild(wrapper);
  const list = [
    { id: 'b', wrapper, wrapperSegments: [wrapper], selectedHtml: '', selectedText: '', comment: '', fragmentUrl: '#b', createdAt: 1 },
    { id: 'a', wrapper, wrapperSegments: [wrapper], selectedHtml: '', selectedText: '', comment: '', fragmentUrl: '#a', createdAt: 1 }
  ];
...
it('returns disconnected segments unchanged when merge spans different block containers', () => {
  ...
  const merged = (manager as unknown as {
...
it('keeps detached wrappers flattened safely and preserves document order for disconnected pairs', () => {
  ...
  const list = [
    { id: 'left', wrapper: detachedA, wrapperSegments: [detachedA], selectedHtml: '', selectedText: '', comment: '', fragmentUrl: '#left', createdAt: 1 },
    { id: 'right', wrapper: detachedB, wrapperSegments: [detachedB], selectedHtml: '', selectedText: '', comment: '', fragmentUrl: '#right', createdAt: 1 }
  ];
  ...
  expect(() =>
    (manager as unknown as { flattenNestedHighlightMarks: (wrapper: HTMLElement) => void }).flattenNestedHighlightMarks(wrapper)
  ).not.toThrow();
});
...
it('returns original connected segments when merge extraction throws', () => {
  ...
  expect(merged).toEqual([first, second]);
  expect(warnSpy).toHaveBeenCalledWith('[ReaderHighlightManager] Failed to merge highlight segments:', expect.any(Error));
  extractSpy.mockRestore();
  warnSpy.mockRestore();
});
