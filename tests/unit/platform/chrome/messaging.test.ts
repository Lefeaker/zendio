  ...
  runtime: {
    ...
    onMessage: {
      addListener: vi.fn((listener: typeof runtimeMessageListener) => {
        runtimeMessageListener = listener ?? undefined;
      }),
...
describe('chromeMessagingService', () => {
  beforeEach(() => {
    ...
    chromeApi.runtime.sendMessage.mockImplementation(
      (_msg: unknown, cb: (response?: unknown) => void) => cb({ pong: true })
    );
    chromeApi.tabs.sendMessage.mockImplementation(
      (_id: number, _msg: unknown, _opt: { frameId?: number }, cb: (response?: unknown) => void) =>
        cb({ tab: true })
    );
  });
  ...
  it('sends runtime and tab messages and supports sync listeners', async () => {
    ...
    await expect(chromeMessagingService.sendToTab(1, { ping: true })).resolves.toEqual({
      tab: true
    });
    ...
    expect(
      runtimeMessageListener?.(
        { ping: true },
        { id: 'ext', tab: { id: 1, url: 'https://example.com' } as chrome.tabs.Tab, frameId: 2 },
        sendResponse
      )
    ).toBe(false);
    expect(sendResponse).toHaveBeenCalledWith({
      message: { ping: true },
      sender: { id: 'ext', tabId: 1, frameId: 2, url: 'https://example.com' }
    });
    off();
  });
});
...
function normalizeMessagePayload(message: unknown): MessagePayload {
  if (
    ...
    message !== null &&
    typeof (message as { ping?: unknown }).ping === 'boolean'
  ) {
