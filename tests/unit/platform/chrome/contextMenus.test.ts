...
let clickedListener:
  | ((info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab | null) => void)
  | undefined;
let shownListener:
  | ((info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab | null) => void)
  | undefined;
const chromeApi = vi.hoisted(() => ({
  contextMenus: {
    ...
    refresh: vi.fn(),
    onClicked: {
      addListener: vi.fn((listener: typeof clickedListener) => {
        clickedListener = listener ?? undefined;
      }),
      removeListener: vi.fn()
    },
    onShown: {
      addListener: vi.fn((listener: typeof shownListener) => {
        shownListener = listener ?? undefined;
      }),
      removeListener: vi.fn()
    }
  }
...
describe('chromeContextMenusService', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    clickedListener = undefined;
    shownListener = undefined;
    chromeApi.contextMenus.create.mockReturnValue('menu-1');
    chromeApi.contextMenus.update.mockImplementation(
      (_id: string, _props: Record<string, unknown>, cb: () => void) => cb()
    );
