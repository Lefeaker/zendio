...
const chromeApi = vi.hoisted(() => ({
  tabs: {
    ...
    onActivated: { addListener: vi.fn((listener: (info: chrome.tabs.OnActivatedInfo) => void) => { activatedListener = listener; }), removeListener: vi.fn() },
...
describe('chromeTabsService', () => {
  beforeEach(() => {
    vi.resetModules(); vi.clearAllMocks(); activatedListener = undefined;
    chromeApi.tabs.create.mockImplementation((props: chrome.tabs.CreateProperties, cb: (tab?: chrome.tabs.Tab) => void) => cb(createTab({ id: 1, ...props })));
    chromeApi.tabs.remove.mockImplementation((_id: number, cb: () => void) => cb());
    chromeApi.tabs.getCurrent.mockImplementation((cb: (tab?: chrome.tabs.Tab) => void) => cb(createTab({ id: 2 })));
    chromeApi.tabs.get.mockImplementation((id: number, cb: (tab?: chrome.tabs.Tab) => void) => cb(createTab({ id })));
    chromeApi.tabs.query.mockImplementation((_info: chrome.tabs.QueryInfo, cb: (tabs: chrome.tabs.Tab[]) => void) => cb([createTab({ id: 3 })]));
    chromeApi.tabs.sendMessage.mockImplementation((_id: number, _msg: unknown, _opt: chrome.tabs.MessageSendOptions, cb: (response?: unknown) => void) => cb({ ok: true }));
  });

  it('wraps tab CRUD messaging and listeners', async () => {
    const { chromeTabsService } = await import('../../../../src/platform/chrome/tabs');
    await expect(chromeTabsService.create({ url: 'https://example.com' })).resolves.toMatchObject({ id: 1 });
    ...
    await expect(chromeTabsService.query({})).resolves.toEqual([expect.objectContaining({ id: 3 })]);
  ...
  it('rejects create when chrome reports a lastError', async () => {
    ...
    await expect(chromeTabsService.create({ url: 'https://example.com' })).rejects.toMatchObject({ message: 'boom' });
  });
