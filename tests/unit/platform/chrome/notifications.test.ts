...
describe('chromeNotificationsService', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    chromeApi.notifications.create.mockImplementation(
      (_id: string, _options: NotificationOptions, cb: (notificationId: string) => void) =>
        cb('notif-1')
    );
    chromeApi.notifications.clear.mockImplementation(
      (_id: string, cb: (wasCleared: boolean) => void) => cb(true)
    );
  });
