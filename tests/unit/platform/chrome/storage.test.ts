...
interface ChangeListener { (changes: Record<string, chrome.storage.StorageChange>, area: string): void }
...
const chromeApi = vi.hoisted(() => ({
  storage: {
    ...
    onChanged: {
      addListener: vi.fn((listener: ChangeListener) => { changeListener = listener; }),
...
describe('chromeStorageService', () => {
  beforeEach(() => {
    ...
    chromeApi.storage.local.get.mockImplementation((key: string | string[], cb: (items: Record<string, unknown>) => void) => cb({ [Array.isArray(key) ? key[0] : key]: 'value' }));
    chromeApi.storage.local.set.mockImplementation((_entries: Record<string, unknown>, cb: () => void) => cb());
    chromeApi.storage.local.remove.mockImplementation((_key: string | string[], cb: () => void) => cb());
