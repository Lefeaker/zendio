...
describe('analytics-delivery-smoke script', () => {
  ...
  it.each([
    ...
    'https://www.google-analytics.com./mp/collect',
    'https://www.google-analytics.com/%6d%70/%63ollect'
  ])('rejects direct Google Measurement Protocol endpoints: %s', async (proxyEndpoint) => {
    const module = await loadModule();
    const logger = createLogger();
    const fetchMock: FetchMock = vi.fn();
    ...
    const result = await module.runAnalyticsDeliverySmoke({
      env: createEnv({
        AIIINOB_GA_MEASUREMENT_ID: 'G-SMOKE1234',
        AIIINOB_GA_TRANSPORT_MODE: 'proxy',
        AIIINOB_GA_PROXY_ENDPOINT: proxyEndpoint
      }),
      fetchImpl: fetchMock,
      stdout: logger.stdout,
      stderr: logger.stderr
  ...
  it('allows non-Google owner proxy paths that resemble Measurement Protocol', async () => {
    const module = await loadModule();
    const logger = createLogger();
    const fetchMock: FetchMock = vi.fn();
